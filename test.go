...
func hapusFilm(id int) {
	var idx int = seqSearchFilmByID(id)
	if idx != -1 {
...
func menuKelolaJadwal() {
	...
	if pilihan == 1 {
		if nJadwal < NMAX {
			...
			if seqSearchFilmByID(jdl.FilmID) != -1 {
				fmt.Print("Waktu Tayang (cth: 14:00): ")
...
func pesanTiket() {
	...
	idxJadwal := seqSearchJadwalByID(jadwalID)
	if idxJadwal != -1 {
		...
		if jumlah > 0 && jumlah <= sisaKursi {
			...
			idxFilm := seqSearchFilmByID(tabJadwal[idxJadwal].FilmID)
			if idxFilm != -1 {
	...
	if judul != "-" {
		...
		for left <= right {
			...
			if info[mid].Judul == judul {
				count++
				fmt.Printf("%s | %s | %d menit\n", info[mid].Judul, info[mid].Genre, info[mid].Durasi)
				left = right + 1
	...
	if genre != "-" {
		for i = 0; i < n; i++ {
			if info[i].Genre == genre && info[i].Judul != judul {
				count++
				fmt.Printf("%s | %s | %d menit\n", info[i].Judul, info[i].Genre, info[i].Durasi)
			}
		}
	}
